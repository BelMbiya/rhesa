<div class="max-w-2xl mx-auto mt-10 p-8 bg-white shadow-md rounded">
    ...
    <h2 class="text-2xl font-bold text-center mb-2 text-[#12458f]">Bienvenue à l'{{ Str::ucfirst($hotel_name) }}</h2>
    <p class="text-center text-[#12458f] mb-8">Remplissez cet formulaire, avant de profitez pleinement de votre sejour !</p>

    <!-- Stepper UI -->
    <div class="relative flex flex-col items-center justify-center mb-10 w-11/12 sm:w-full max-w-3xl mx-auto min-h-[120px]">
        <div class="flex justify-center items-center w-full relative px-[5%] md:px-[18%]">
            <div class="flex justify-between w-full relative z-10">
                @for ($i = 1; $i <= 3; $i++)
                    <div class="flex flex-col items-center {{ $i === 2 ? '-translate-x-[10%] sm:-translate-x-[10%]' : '' }}">
                        ...
                        <div class="mt-2 text-sm {{ $currentStep == $i ? 'font-semibold text-[#12458f]' : 'text-gray-500' }}">
                            @switch($i)
    ...
    <!-- Contenu des étapes -->
    @if ($currentStep === 1)
        <x-input label="Numéro d'indentité" wire:model.defer="identity_number" autofocus />
        <div class="flex flex-row justify-between mt-6 mb-6 gap-4">
            ...
            <input type="file" wire:model.defer="selfi" x-ref="selfiInput" class="hidden" accept="image/*" capture="user" key="{{ now() }}" />
            ...
            <div class="w-full max-w-[42%] p-3 bg-white rounded-xl shadow-md border border-blue-200">
                ...
                @if ($identity_image)
                    <div class="flex items-center mt-4 px-4 py-2 bg-gray-100 rounded-md">
                        ...
                        <span class="text-sm text-gray-600 truncate">{{ $identity_image->getClientOriginalName() }}</span>
                    </div>
                @endif
            </div>
            <input type="file" wire:model.defer="identity_image" x-ref="identity_imageInput" class="hidden" accept="image/*" capture="environement" key="{{ now() }}" />
            @error('identity_image') <span class="text-red-500 text-sm mt-2">{{ $message }}</span> @enderror
        </div>

    @elseif ($currentStep === 2)
        <div>
            <x-input label="Nom" wire:model.defer="First_name" autofocus />
            <x-input label="Postom" wire:model.defer="Last_name" />
            <x-select label="Sexe" wire:model.defer="gender_id">
                <option value="">Sélectionner...</option>
                @foreach ($genres_id as $genre)
                    <option value="{{ $genre->id }}">{{ $genre->name }}</option>
                @endforeach
            </x-select>
            <x-input label="Date de naissance" wire:model.defer="date_naissance" type="date" />
            <x-input label="Lieu de naissance" wire:model.defer="lieu_naissance" />
            {{-- ✅ wire:model sans defer pour déclencher updatedNationalite en temps réel --}}
            <x-select label="Nationalité" wire:model="nationalite">
                <option value="">Sélectionner...</option>
                <option value="Congolaise">Congolaise</option>
                <option value="Etrangere">Etrangère</option>
            </x-select>
            <x-input label="Adresse" wire:model.defer="adresse" />
            <x-input label="Téléphone" wire:model.defer="telephone" />
            <x-input label="Email" wire:model.defer="email" type="email" />
            <x-input label="Profession" wire:model.defer="profession" />
            <x-input label="Nom du père" wire:model.defer="father_name" />
            <x-input label="Nom de la mère" wire:model.defer="mother_name" />
            <x-input label="Enfants de moins de 15ans" wire:model.defer="children_under_15" />
        </div>

    @elseif ($currentStep === 3)
        <div class="min-h-[550px] md:min-h-[450px]">
            {{-- ✅ Affiché uniquement si nationalité Etrangere --}}
            @if ($nationalite === 'Etrangere')
                <div>
                    <x-input label="Date d'arrivée au pays" wire:model.defer="arrival_country_date" type="date" />
                    <x-input label="Date de depart du pays" wire:model.defer="departure_country_date" type="date" />
                </div>
            @endif

            <x-input label="Numero de la chambre" wire:model.defer="num_chambre" type="text" />

            <div class="w-full max-w-[42%] p-3 bg-white rounded-xl shadow-md border border-blue-200">
                <div class="flex flex-col items-center justify-center border-2 border-dashed border-blue-400 rounded-md h-32 cursor-pointer"
                    wire:click="$refs.signatureInput.click()" @click="$refs.signatureInput.click()">
                    @if (!$signature)
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8 text-blue-500 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                        </svg>
                        <span class="text-sm text-gray-500">Votre signature</span>
                    @else
                        <img src="{{ $signature->temporaryUrl() }}" alt="Preview" class="object-cover h-full w-full rounded-md" />
                    @endif
                </div>
                @if ($signature)
                    <div class="flex items-center mt-4 px-4 py-2 bg-gray-100 rounded-md">
                        ...
                        <span class="text-sm text-gray-600 truncate">{{ $signature->getClientOriginalName() }}</span>
                    </div>
                @endif
            </div>
            <input type="file" wire:model.defer="signature" x-ref="signatureInput" class="hidden" accept="image/*" capture="environement" key="{{ now() }}" />
            @error('signature') <span class="text-red-500 text-sm mt-2">{{ $message }}</span> @enderror
        </div>
    @endif
